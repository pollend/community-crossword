...
pub fn main() !void {
    ...
    game.state = .{
        ...
        .client_pool = try .initCapacity(allocator, 1024),
    ...
    var reader = crossword_file.reader(thread.io(), &file_buffer);
    game.state.board = try game.Board.load(allocator, &reader.interface, null);
    game.state.global = game.HighscoreTable100.init(allocator);
    var threaded: std.Io.Threaded = .init(allocator);
    defer threaded.deinit();

    var main_group: std.Io.Group = .init;
    const io = threaded.io();
    try main_group.concurrent(io,  game.background_worker, .{io});
