...
pub const Entry = struct {
    ...
    last_word_solve: bounded_array.BoundedArray(game.Value, 64),
...
pub fn FixedHighscoreTable(comptime size: usize) type {
    return struct {
        ...
        pub fn init(allocator: std.mem.Allocator) Self {
            return .{ .lock = .{}, .table = undefined, .num_entries = 0, .allocator = allocator };
        }
        ...
        pub fn decode(allocator: std.mem.Allocator, reader: *std.Io.Reader) !Self {
            var result = Self.init(allocator);
            const magic = try reader.readInt(u32, .little);
            if (magic != HIGHSCORE_MAGIC_NUMBER) {
                return error.InvalidHighscoreMagicNumber;
        ...
        pub fn encode(self: *Self, writer: *std.Io.Writer) !void {
            self.lock.lock();
        ...
        pub fn update_nick(self: *Self, profile_id: u64, nick: []const u8) void {
            ...
            while (found_index < self.num_entries) : (found_index += 1) {
                if (self.keys[found_index] == profile_id) {
                    self.table[found_index].nick.len = 0; // .resize(0) catch unreachable;
