...
pub fn update_solved(self: *ProfileSession, cl: *game.Clue) void {
...
pub fn parse_cookie(allocator: std.mem.Allocator, key: [Aegis256.key_length]u8, cookie: []const u8) !ProfileSession {
    ...
    var buffer = try allocator.alloc(u8, try b64.calcSizeForSlice(data_b64));
    defer allocator.free(buffer);
    var decrypted_buffer = try allocator.alloc(u8, try b64.calcSizeForSlice(data_b64));
    ...
    try Aegis256.decrypt(decrypted_buffer, buffer[0..], tag, &empty_buf, nonce, key);

    var reader: std.Io.Reader = .fixed(decrypted_buffer[0..]);
...
pub fn create_cookie(self: *ProfileSession, allocator: std.mem.Allocator, key: [Aegis256.key_length]u8) ![]u8 {
    ...
    errdefer allocator.free(cookie_buffer);
...
pub fn load_session(reader: *std.Io.Reader) !ProfileSession {
...
test "serialize and deserialize" {
    ...
    var clue = try game.Clue.init_from_ascii(allocator, "test", "A test clue", .{ 0, 0 }, .Across);
    defer clue.deinit();
...
test "verify cookie creation and parsing" {
    const allocator = std.testing.allocator;
    var session = ProfileSession.empty(20);

    var clue = try game.Clue.init_from_ascii(allocator, "example", "An example clue", .{ 0, 0 }, .Down);
    defer clue.deinit();
    session.update_solved(&clue);

    var key: [Aegis256.key_length]u8 = undefined;
    std.crypto.random.bytes(key[0..]);

    const cookie = try session.create_cookie(allocator, key);
    defer allocator.free(cookie);

    const parsed_session = try ProfileSession.parse_cookie(allocator, key, cookie);

    try std.testing.expectEqual(session.profile_id, parsed_session.profile_id);
    try std.testing.expectEqual(session.num_clues_solved, parsed_session.num_clues_solved);
    try std.testing.expectEqual(session.score, parsed_session.score);
}
